...
for ax, df, title in zip(axes, [l1, l2, l3],
    ...
    p = piv(df, "p_collapse")
    im = ax.imshow(p.values, origin="lower", cmap="cividis", vmin=0, vmax=1, aspect="auto")
    ax.set_xticks(range(len(M_VALS)))
    ax.set_xticklabels(M_VALS, fontsize=14)
    ax.set_yticks(range(len(J_VALS)))
    ax.set_yticklabels([f"{j:.1f}" for j in J_VALS], fontsize=14)
    ax.set_xlabel("income multiplier", fontsize=15)
    if ax is axes[0]:
        ax.set_ylabel("coupling J", fontsize=15)
    ax.set_title(title, fontsize=14)
    for i in range(p.shape[0]):
        for j in range(p.shape[1]):
            v = p.values[i, j]
            ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                    color=_txt_color("cividis", v), fontsize=15)
    fig.colorbar(im, ax=ax, fraction=0.045, label="P(collapse)")
fig.suptitle("Ablation cascade: P(collapse) heatmap survives across L1 → L2 → L3", fontsize=17)
fig.tight_layout()
...
for ax, df, title in zip(axes, [l1, l2, l3],
    ...
    for i in range(p.shape[0]):
        for j in range(p.shape[1]):
            v = p.values[i, j]
            ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                    color=_txt_color("Reds", v), fontsize=15)
    fig.colorbar(im, ax=ax, fraction=0.045)
fig.suptitle("Rigidity collapses dominate the high-J residual at every level", fontsize=17)
fig.tight_layout()
...
high-J residual collapse rate (L1 {stats['L1']['high_J_max_mult']:.0%} → L2 {stats['L2']['high_J_max_mult']:.0%} → L3 {stats['L3']['high_J_max_mult']:.0%} at max
