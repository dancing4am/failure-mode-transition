"""
Generate Figure 4 + supporting figures + NETWORK_VERDICT.md from the
...
def figure_pcollapse_comparison(df):
    ...
    fig.suptitle("P(collapse) across network topologies "
...
def write_verdict(df):
    ...
    if p_modular < 0.05 and rel_drop_mod > 0.7:
        ...
        scenario_text = (
            ...
            "the high-coupling residual does not appear on modular topologies"
        )
    elif p_modular < p_complete * 0.6 or rel_drop_mod > 0.4:
        scenario = "B"
        scenario_text = (
            "modularity PARTIALLY restores passive stabilizer effectiveness — "
            "the high-coupling residual is preserved but reduced"
        )
    else:
        scenario = "A"
        scenario_text = (
            "the high-coupling residual SURVIVES on all tested topologies — "
            "the mean-field result is topology-robust (these are 50-seed values; the manuscript's cited 0.33–0.37 band is the superseding 100-seed re-run in headline_100seed.csv)"
        )
    ...
    with out.open("w", encoding="utf-8") as f:
        ...
        # Committed run yields scenario A (the high-coupling residual SURVIVES on all tested
        # topologies, matching the manuscript). Scenarios B/C below are non-firing conditional
        # branches retained for alternative data; their "restores" wording is not a current claim.
        if scenario == "A":
            f.write(
                "The high-coupling residual is robust to network topology "
                ...
                "(~150 words) + Figure 4 (the 6-panel heatmap). Update "
            ...
            f.write(
                ...
                "eliminate the high-coupling residual; passive stabilizers gain "
                "effectiveness at module boundaries when the modules "
                "themselves decouple.\n\n"
                "**Recommended integration:** new Results subsection "
                "\"Network architecture as a passive-stabilizer "
                "amplifier\" with Figure 4. The paper's central claim "
                "is preserved — the high-coupling residual is the upper bound on "
            ...
            f.write(
                ...
                "and the high-coupling residual does not bind. This is a substantial "
                ...
                "the high-coupling-residual result as 'an upper bound on passive failure in "
