"""Dynamic J(t) — time-dependent coupling experiment on the minimal model.

J ramps over time to model gradual coupling tightening. The question is:
...
# Reproducibility guard: the seed formula below folds Python's hash() of the
# schedule name into the seed. String hashing is randomized per process unless
# PYTHONHASHSEED is pinned, so re-exec with PYTHONHASHSEED=0 for bit-exact runs.
import os as _os
import sys as _sys
if _os.environ.get("PYTHONHASHSEED") != "0":
    ...
    else:  # os.execve is unreliable on Windows (spawn semantics); re-run instead
        import subprocess as _subprocess
        raise SystemExit(_subprocess.call([_sys.executable] + _sys.argv, env=_env))
...
# Models a discontinuous coupling increase (sudden-onset ramp).
...
def main():
    ...
    fig.suptitle("Survival curves under dynamic J(t)\n"
