...
Question: between the gradual (30k-step) and sudden (~100-step) extremes
...
Holds total simulation horizon fixed at N_STEPS = 40,000 so that even the
slowest ramp (30k) has 10,000 post-ramp settling steps for collapse to
...
N_SEEDS = 100
N_STEPS = 40_000  # 2,000 physical time units; slowest ramp (30,000 steps) + 10,000 settling steps
...
def main():
    ...
    ax.set_title("Critical ramp speed: P(collapse) vs ramp duration\n"
                 "slower ramps protect only above the viability margin; "
                 "at $\\mu \\leq 50$ they raise collapse")
    ax.set_ylim(-0.02, 1.05)
    ax.grid(alpha=0.3, which="both")
    # legend outside the axes (right) so it never occludes the mu=40 curve
    ax.legend(fontsize=9, ncol=1, loc="upper left",
              bbox_to_anchor=(1.01, 1.0), borderaxespad=0.0)
    fig.tight_layout()
    fig.savefig(OUT / "collapse_vs_ramp_speed.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # --- Figure B: collapse-type composition vs ramp_steps -----------------
    # Only margins with appreciable collapse counts (mu <= 60): mu >= 65 have
    # <= 1 collapsed seed, so their composition is not meaningful. Shown with
    # fewer, wider panels and larger fonts so the panel titles, the n= counts,
    # and the tick labels are legible at print size.
    COMP_MULTS = tuple(m for m in MULT_VALUES if m <= 60)
    fig, axes = plt.subplots(1, len(COMP_MULTS), figsize=(4.2 * len(COMP_MULTS), 4.6),
                              sharey=True)
    width = 0.7
    for ax, mult in zip(axes, COMP_MULTS):
        sub = summary_df[summary_df["mult"] == mult].sort_values("ramp_steps")
        x = np.arange(len(sub))
        ...
        ax.set_title(f"mult={mult}", fontsize=16)
        ax.set_ylim(0, 1.12)
        ax.tick_params(axis="y", labelsize=13)
        # n= counts staggered vertically so adjacent "n=100" labels do not collide
        for k, (xi_, t_n) in enumerate(zip(x, total)):
            ax.text(xi_, 1.02 + 0.045 * (k % 2), f"n={int(t_n)}",
                    ha="center", fontsize=11, color="grey")
    axes[0].set_ylabel("collapse-type fraction\n(of collapsed seeds)", fontsize=14)
    # legend outside the last panel so it never sits on the mult=60 bars
    axes[-1].legend(fontsize=13, loc="upper left",
                    bbox_to_anchor=(1.02, 1.0), borderaxespad=0.0)
    fig.suptitle(r"Collapse-type composition by ramp duration "
                 r"(margins with appreciable collapse counts, $\mu \leq 60$)"
                 "\nmixed dominates short ramps at $\\mu$ = 40, 50 (rigidity at $\\mu$ = 60); "
                 "fragmentation appears at slow ramps + low mult",
                 fontsize=15)
    fig.tight_layout()
    fig.savefig(OUT / "collapse_types_vs_ramp.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
